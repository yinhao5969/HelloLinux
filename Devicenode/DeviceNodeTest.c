...
/*设备为杂项设备，驱动使用 platform_driver_register 注册，设备使用
 * platform_device_register 注册，设备节点由 misc_register 生成*/
...
static int DeviceNodeTest_Device_init(void) {
    ...
    printk(KERN_EMERG
           "platform_device_register try to registetr DeviceNodeTest_Device\n");
...
static void DeviceNodeTest_Device_exit(void) {
    ...
    printk(
        KERN_EMERG
        "platform_device_unregister try to unregister DeviceNodeTest_Device\n");
...
// misc device defenition
// misc device defenition
static long hello_ioctl(struct file *files, unsigned int cmd,
                        unsigned long arg) {
...
static int DeviceNodeTest_Driver_probe(struct platform_device *pdv) {
    ...
    printk(KERN_EMERG
           "misc_register try to register DeviceNodeTest_MiscDevice\n");
...
static int DeviceNodeTest_Driver_remove(struct platform_device *pdv) {
    ...
    printk(KERN_EMERG
           "misc_deregister try to unregister DeviceNodeTest_MiscDevice\n");
...
static int DeviceNodeTest_Driver_suspend(struct platform_device *pdv,
                                         pm_message_t pmt) {
...
static int DeviceNodeTest_init(void) {
    ...
    // creat driver
    printk(KERN_EMERG "ENTER: platform_driver_register try to register "
                      "DeviceNodeTest_Driver\n");
...
static void DeviceNodeTest_exit(void) {
    ...
    printk(KERN_EMERG "platform_driver_unregister try to unregister "
                      "DeviceNodeTest_Driver!\n");
