...
struct platform_device LedCtl_Device = {.name = DEVICE_NAME,
                                        .id = -1,
                                        .dev = {
...
static int LedCtl_Device_init(void) {
    ...
    printk(KERN_EMERG
           "platform_device_register try to registetr LedCtl_Device\n");
...
static void LedCtl_Device_exit(void) {
    ...
    // unregister device
    printk(KERN_EMERG
           "platform_device_unregister try to unregister LedCtl_Device\n");
    platform_device_unregister(&LedCtl_Device);
    return;
}

// misc definition
static long LedCtl_ioctl(struct file *files, unsigned int cmd,
                         unsigned long arg) {
...
static int LedCtl_resume(struct platform_device *pdv) { return 0; }

struct platform_driver LedCtl_driver = {.probe = LedCtl_probe,
...
static int LedCtl_init(void) {
    ...
    // creat driver
    printk(KERN_EMERG
           "ENTER: platform_driver_register try to register LedCtl_driver\n");
...
static void LedCtl_exit(void) {
    ...
    printk(KERN_EMERG
           "platform_driver_unregister try to unregister LedCtl_driver!\n");
